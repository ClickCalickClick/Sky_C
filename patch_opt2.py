...
start_str = "  for (int16_t y = 0; y < height; y += step) {"
end_str =   "    }\n  }"
match = re.search(re.escape(start_str) + r'.*?' + re.escape(end_str), text, re.DOTALL)

if match:
    print("Found loop regex")
    
    new_loop = """  for (int16_t y = 0; y < height; y += step) {
    ...
    # We must only replace the SECOND match of the loop! Because prv_sample_gradient_color doesn't have these loops.
    # Actually prv_sample_gradient_color doesn't have the loop! It has no X/Y loops.
    
    with open('src/c/Sky.c', 'w') as f:
        f.write(text.replace(match.group(0), new_loop))
else:
    print("Failed to find loop natively via regex.")
